...
make_filtered_sce <- function(sample_name) {
  counts <- Read10X_h5(here("../flex_multiplets/data", "10X_PBMC_16plex", "HDF5", 
...
sce <- scDblFinder(sce, clusters = T,
                   BPPARAM=bp, 
                   dbr.sd = 1,) #dbr.sd = 1 does not assume the doublet rate, this has changed since v1 of paper. 
                   samples = sce$sample_barcode)
```
```{r}
doublets_no_samples <- sce$scDblFinder.class
```
